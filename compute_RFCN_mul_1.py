...
if __name__ == '__main__':
    ...
    prototxt = '/home/pww/test/git/fd_frcn_rfcn/models/pascal_voc/ResNet-101/rfcn_end2end/test_agonistic.prototxt'
     
    #caffemodel = os.path.join( args.demo_net )
    
    caffemodel = '../data/rfcn_models/resnet101_rfcn_ohem_iter_40000.caffemodel'
    ...
    output = open('result-DXP-0.6-1.25.txt', 'w')
        ...
        for label in open(label_file_path):
            ...
            for i, d in enumerate(bbox):
                ...
                if f is None:
                    ...
                    for j in range(-100, -20, 2)[::-1]:
                        ran.append(j * 0.1)
                    for j in range(22, 102, 2):
                        ran.append(j * 0.1)
                    for j in ran:
                        pad = [int((d[2]-d[0]) / j), int((d[3]-d[1]) / j)]
