    ...
    def BaseFeat(self, leader):
        ...
        for path in bases:
            ...
            if bbs[0][6] == '1':
                ...
                img_fro[path.split('/')[-1]] = nimg
                self.num_img[path.split('/')[-1]] = 0
            else:
...
if __name__ == '__main__':
    ...
    mod_vec2vec.bind(for_training=False, data_shapes=[('data', (1, 3, 224, 112))],
                     label_shapes=mod_vec2vec._label_shapes)
    ...
    for leader in cfg.leaders_sml[:-1]:
        ...
        for type in ['pro']:
            try:
                ...
                os.mkdir(save_root + type +'/result/')
                os.mkdir(save_root + type + '/right0/')
                os.mkdir(save_root + type + '/right1/')
                os.mkdir(
                    save_root + type + '/falseNegative')  # groundturth is same person, recognized to be different person
            ...
            for i, d in enumerate(bbox):
                ...
                if pose == 1:  ## use vec2vec model when assured profiles
                    ...
                    for leader in cfg.leaders_sml[:-1]:
                        mean_match[leader] = 0
                        num_match[leader] = 0
                        for base_name in cfg.img_box_fro[leader]:
                            f_img = cfg.img_box_fro[leader][base_name]
                            ...
                            if prob[0] >= 0.9:
                                ...
                                num_match[leader] += 1
                                cfg.num_img[base_name] += 1
                                save_name = save_root + 'pro/result/{}_{}_{}_{:.3f}.jpg'.format(img_name[:-4], leader, num_match[leader], prob[0])
                                if not cv2.imwrite(save_name, img_contate):
                                    print('save failed')
                                    exit(0)
                        if num_match[leader] != 0:
                            mean_match[leader] /= num_match[leader]
                        if max_mean < mean_match[leader] and mean_match[leader] > 0.9:
                            result_v2v = leader
                    ...
                    if result_v2v == positive:
                        if positive != 'OTHERS':
                            if FLAG_F:
                                ...
                                right_pro[0] += 1
                            if not cv2.imwrite(
                                    '{}{}/right0/{}_p_{}.jpg'.format(save_root, type, img_name[:-4], i), nimg):
                                print(
                                'save \'{}{}/right0/{}_p_{}.jpg\' error'.format(save_root, type, img_name[:-4],
                                                                                i))
                                exit(0)
                        else:
                            if not FLAG_F:
                                ...
                                right_pro_fu[1] += 1
                            if not cv2.imwrite(
                                    '{}{}/right1/{}_p_{}.jpg'.format(save_root, type, img_name[:-4], i),
                                    nimg):
                                FLAG_SAVE = False
                        ...
                        if FLAG_F:
                            ...
                            fn_pro += 1
                        if not cv2.imwrite(
                                '{}{}/falseNegative/{}_p_{}_{}.jpg'.format(save_root, type, img_name[:-4], i,
                                                                           result_v2v),
                                nimg):
                            FLAG_SAVE = False
                    else:
                        if FLAG_F:
                            ...
                            fp_pro += 1
                        if not cv2.imwrite(
                                '{}{}/falsePositive/{}_p_{}_{}.jpg'.format(save_root, type, img_name[:-4], i,
                                                                           result_v2v),
                                nimg):
                            FLAG_SAVE = False
                    if not FLAG_SAVE:
